    ...
    <tr>
        ...
        @foreach($table->columns() as $column)
            <th v-show="table.columnIsVisible(@js($column->key))"
                class="@if($loop->first && $hasBulkActions) px-2 @else px-2 @endif py-2.5 text-left text-xs border-l dark:border-stone-800 font-medium tracking-wide text-stone-400 {{ $column->classes }}"
            >
                @if($column->sortable)
                ...
                <span class="flex flex-row items-center @if($column->alignment == 'right') justify-end @elseif($column->alignment == 'center') justify-center @else justify-start @endif">
                    ...
                    @if($column->sortable)
                        <svg aria-hidden="true" class="w-3 h-3 ml-2 @if($column->sorted) text-{{ config('boilerplate.color.label') }}-500 @else text-stone-400 @endif" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512">
                            @if(!$column->sorted)
