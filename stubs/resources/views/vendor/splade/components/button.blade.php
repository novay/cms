...
<SpladeButton {{ $attributes->only(['v-bind:spinner', ':spinner']) }}>
    <template #default="button">
        <{{ $type === 'link' ? 'Link' : 'button' }} :disabled="button.spinner" {{ $attributes->class([
            ...
            'bg-white hover:bg-stone-100 text-stone-700 border-stone-300 focus:border-yellow-300 focus:ring-yellow-200' => !$customStyling && $secondary,
        ])
        ...
        >
            ...
            @else
                <div class="flex flex-row items-center justify-center">
                    <svg
                        v-if="button.spinner"
                        class="animate-spin mr-3 h-5 w-5 @if($secondary) text-stone-700 @else text-white @endif" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                    >
