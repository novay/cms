...
>
    <template #default="{!! $scope !!}">
        <label class="block space-y-1" v-bind:class="{ 'pointer-events-none': select.loading }">
            @includeWhen($label, 'splade::form.label', ['label' => $label])

            <div class="relative">
                <div v-bind:class="{ 'opacity-50': select.loading }">
                    <select {{ $attributes->except(['v-if', 'v-show', 'class'])->class([
                        'block w-full rounded-md border-stone-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 disabled:opacity-50',
                    ])->merge([
                        'multiple' => $multiple,
                        'name' => $name,
